...
namespace Crc
{
static const unsigned char CRC8_INIT_JUDGE = 0xff;
// crc8 verification table
static const unsigned char CRC8_TAB_JUDGE[256] = {
...
static const uint16_t CRC_INIT_JUDGE = 0xffff;
// crc16 verification table
static const uint16_t wCRC_Table_JUDGE[256] = {
...
unsigned int verifyCRC8CheckSum(unsigned char *msg, unsigned int len)
{
    ...
    if ((msg == 0) || (len <= 2))
        return 0;
    ucExpected = getCRC8CheckSum(msg, len - 1, CRC8_INIT_JUDGE);
    return (ucExpected == msg[len - 1]);
}
...
void appendCRC8CheckSum(unsigned char *msg, unsigned int len)
{
    ...
    if ((msg == 0) || (len <= 2))
        return;
    ucCRC        = getCRC8CheckSum((unsigned char *)msg, len - 1, CRC8_INIT_JUDGE);
    msg[len - 1] = ucCRC;
}
...
uint16_t getCRC16CheckSum(uint8_t *msg, uint32_t len, uint16_t wCRC)
{
    ...
    {
        chData = *msg++;
        (wCRC) = ((uint16_t)(wCRC) >> 8) ^ wCRC_Table_JUDGE[((uint16_t)(wCRC) ^ (uint16_t)(chData)) & 0x00ff];
    }
    return wCRC;
}
...
uint32_t verifyCRC16CheckSum(uint8_t *msg, uint32_t len)
{
    ...
    wExpected = getCRC16CheckSum(msg, len - 2, CRC_INIT_JUDGE);
    return ((wExpected & 0xff) == msg[len - 2] && ((wExpected >> 8) & 0xff) == msg[len - 1]);
}
...
void appendCRC16CheckSum(uint8_t *msg, uint32_t len)
{
    ...
    wCRC         = getCRC16CheckSum((uint8_t *)msg, len - 2, CRC_INIT_JUDGE);
    msg[len - 2] = (uint8_t)(wCRC & 0x00ff);
    msg[len - 1] = (uint8_t)((wCRC >> 8) & 0x00ff);
}

/**
 * @brief Crc checksum for CCITT format
 *
 * @param ptr
 * @param len
 * @return uint16_t
 */
uint16_t getCRC16CCITTCheckSum(uint8_t *ptr, int len)
{
    ...
    while (length--)
    {
        crc = (crc >> 8) ^ wCRC_Table_JUDGE[(crc ^ *ptr++) & 0xff];
    }

    return crc;
}
/**
 * @brief verify CRC16CCITT checksum
 *
 * @param msg
 * @param len
 * @return uint32_t
...
uint32_t verifyCRC16CCITTCheckSum(uint8_t *msg, uint32_t len)
{
    ...
    wExpected = getCRC16CCITTCheckSum(msg, len - 2);
    return ((wExpected & 0xff) == msg[len - 2] && ((wExpected >> 8) & 0xff) == msg[len - 1]);
}
/**
 * @brief append CRC16CCITT checksum, len must be greater than 2,length is msg
 * len + 2 CRC bytes
 *
 * @param msg
 * @param len
...
void appendCRC16CCITTCheckSum(uint8_t *msg, const uint32_t &len)
{
    configASSERT(msg != nullptr);
    configASSERT(len <= 1024);
    volatile uint16_t wCRC = 0;
    volatile int length    = len;
    if ((msg == 0) || (len <= 2))
    {
        return;
    }
    wCRC            = getCRC16CCITTCheckSum((uint8_t *)msg, length - 2);
    msg[length - 2] = (wCRC & 0xFF);
    msg[length - 1] = ((wCRC >> 8) & 0xFF);
}
